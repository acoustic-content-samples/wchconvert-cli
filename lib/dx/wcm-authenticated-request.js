...
var httpGetHelper = function(options) {
    ...
    var isFile = options.headers.Accept === 'application/octet-stream';
    var filename = options.filename;
    
    var deferred = Q.defer(), body = '', doRequest = function(options) {
        // logger.debug('httpGetHelper:: options::', options);
        if (options.secure) {
        ...
        var reqGet = http.request(options, function(response) {
            if(options.headers && options.headers.ContentType && options.headers.ContentType){
                ...
                logger.debug(options.headers.ContentType + ' read binary');
            }
            if (response.statusCode === 404) {
                var err = getErrorFromResponse(null, response);
                logger.debug('httpGetHelper::err::' + err);
                deferred.reject(err);
...
var httpPostHelper = function(options, postData) {
    ...
    var deferred = Q.defer(), body = '', doRequest = function(options, postData) {
        logger.debug('httpPostHelper:: options::', options, ' postData::' + postData);
        if (options.secure) {
            options.rejectUnauthorized = false;
        }
        var reqPost = http.request(options, function(response) {
            if (response.statusCode === 404) {
                var err = getErrorFromResponse(null, response);
                logger.error('httpPostHelper::err::' + err);
                deferred.reject(err);
            } else if (response.statusCode > 300 && response.statusCode < 400 && response.headers.location) {
                options.path = response.headers.location;
...
var getLTPAToken = function(user, pass, options, postData) {
    ...
    var deferred = Q.defer(), doRequest = function(optionspost, options, postData) {
        logger.debug('getLTPAToken:: user::' + user + ' options::', options, ' postData::' + postData);
        ...
        try {
            ...
            var reqPost = http.request(optionspost, function(response) {
                if (!response.headers['set-cookie']) {
                    ...
                    logger.debug('getLTPAToken::err::' + err);
                    deferred.reject(err);
                    return;
                }
            ...
            reqPost.end();
            reqPost.on('error', function(e) {
                logger.debug('getLTPAToken::err::' + e);
                deferred.reject(e);
            });
        } catch (err) {
...
var checkStartRequest = function(path) {
    if (warnParallel) {
        ...
        if (currentRequestLevel > 1) {
            logger.warn('Parallel requests detected - level: ' + currentRequestLevel + ' pending request: ' + currentRequestUrl);
        }
    }
...
var authenticatedRequest = function(user, pass, options, postData) {
    var deferred = Q.defer(), authenticate = function(user, pass, options, postData) {
        // logger.debug('authenticatedRequest:: user::' + user + ' options::', options, ' postData::' + postData);
        ...
        logger.debug('authenticated-request for ', options.path);
            ...
            getLTPAToken(user, pass, options, postData).then(function(data) {
                ...
                if (postData === undefined) {
                    return httpGetHelper(options).then(function(data) {
                        // logger.debug('Completed get for ', options.path);
                        checkEndRequest();
                        deferred.resolve(data);
                    }, function(err) {
                        checkEndRequest();
                        if (err.statusCode !== undefined && err.statusCode === success_http_code) {
                            ...
                            logger.debug('authenticatedRequest::err::' + err);
                            deferred.reject(err);
                        }
                    });
                } else {
                    return httpPostHelper(options, postData).then(function(data) {
                        // logger.debug('Completed post for ', options.path);
                        checkEndRequest();
                        deferred.resolve(data);
                    }, function(err) {
                        checkEndRequest();
...
//todo: do the JSON parse here, so each caller doesn't have to do it
var getFile = function(uri, filename) {
    var deferred = Q.defer(), authenticate = function(uri) {
        // logger.debug('getJson:: uri::' + uri);
        ...
        authenticatedRequest(globalUser, globalPassword, callOptions).then(function(data) {
            ...
            if (err.statusCode !== undefined && err.statusCode === success_http_code) {
                ...
                logger.debug('getFile::err::' + err);
                deferred.resolve('FAILED: ' + err.statusCode);
            }
        });
    };
    authenticate(uri);
...
var setJson = function(uri, postData, method) {
    var deferred = Q.defer(), authenticate = function(uri, postData) {
        var headers;
        //logger.debug.trace('setJson:: uri::' + uri + ' postData::' + postData);
...
var setContent = function(uri, contentType, data) {
    var deferred = Q.defer(), authenticate = function(uri, contentType, putData) {
        //logger.debug('setContent:: uri::' + uri + ' contentType::' + contentType + ' data::' + data);
        var callOptions = {
            host : globalHost,
            contentHandlerPath : globalContentHandlerPath,
            port : globalPort,
        ...
        authenticatedRequest(globalUser, globalPassword, callOptions, putData).then(function(data) {
            ...
            if (err.statusCode !== undefined && err.statusCode === success_http_code) {
                ...
                err = getErrorFromResponse(err);
                logger.debug('setContent::err::' + err);
                deferred.reject(err);
...
var getContent = function(uri, contentType) {
    var deferred = Q.defer(), authenticate = function(uri, contentType) {
        //logger.debug('getContent:: uri::' + uri + ' contentType::' + contentType);
        var callOptions = {
            host : globalHost,
            port : globalPort,
            contentHandlerPath : globalContentHandlerPath,
        ...
        authenticatedRequest(globalUser, globalPassword, callOptions).then(function(data) {
            ...
            if (err.statusCode !== undefined && err.statusCode === success_http_code) {
                ...
                err = getErrorFromResponse(err);
                logger.debug('getContent::err::' + err);
                deferred.reject(err);
            }
        }).done();
    };
...
var init = function(host, port, user, password, contentHandlerPath, secure) {
    ...
    //logger.debug('init:: host::' + host + ' port::' + port + ' user::' + user + ' contentHandlerPath::' + contentHandlerPath + ' secure::' + secure);
...
var getErrorFromResponse = function(err, response) {  //jshint ignore: line
    //logger.debug('getErrorFromResponse:: err::' + err + ' response::' + response);
    var error = {};

...
var dxUtils = require('../dx/dxUtils'),
	wchTypeElement = require('./wchTypeElement'),
...
function convert(sourceJson, typeNames) {  // jshint ignore: line
	var targetJson = {};
	...
	var savedName = utils.findByVal(typeNames, id);
...
//	var rev = '';
	...
	for (var index = 0; index < sourceElements.length; index++) {
        ...
        if (label === '') {
            logger.warn('Title not specified for element ' + key + ' in type ' + id + '.  Will use key as label.');
            label = key;
        }
	...
	targetJson = genType(id, name, description);
...
function analyze(sourceJson, typeNames, statMap) { //jshint ignore: line
	...
	if (utils.findByVal(typeNames, id) !== undefined) {
		name = utils.findByVal(typeNames, id);
	...
	var sourceElements = sourceJson.elements;
	if (sourceElements === undefined) {
        statMap.errors.push('Type does not have any elements. TypeID: ' + id);
        sourceElements = [];
    }
	...
	for (var index = 0 ; index < sourceElements.length; index++) {
		...
		if (sourceElement.type.toUpperCase() === 'DATECOMPONENT') {
			logger.debug('Type has ' + sourceElement.type.toUpperCase() + ' component: ' + name);
		}
